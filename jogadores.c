...
for(i = 0; i < total_jogadores; i++){
    for(j = 0; j < total_jogadores - 1; j++){
        if(strcmp(jogadores[j].user, jogadores[j + 1].user) > 0){ // indicar que o jogador atual ? maior que o pr?ximo jogador em ordem alfab?tica, ocorre uma troca de posi??es
            aux = jogadores[j];
